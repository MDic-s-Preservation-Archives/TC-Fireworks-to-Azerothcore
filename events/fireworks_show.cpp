...
public:
    ...
    struct go_cheer_speakerAI : public GameObjectAI
    {
        ...
        {
            _events.Update(diff);
            time_t time = GameTime::GetGameTime().count();
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_FIRE:
                    {
                        if (std::vector<Position> const* positions = Acore::Containers::MapGetValuePtr(PositionsByZoneMap, me->GetZoneId()))
                        {
                            Position const& rndpos = Acore::Containers::SelectRandomContainerElement(*positions);

                            // Extract x, y, z coordinates from rndpos
                            float posX = rndpos.m_positionX;
                            float posY = rndpos.m_positionY;
                            float posZ = rndpos.m_positionZ;

                            // Generate random rotation angles
                            float rndrot = frand(-1.0f, 1.0f);
                            float rndrot2 = frand(-1.0f, 1.0f);

                            // Convert random rotation angles to radians
                            float angle1 = G3D::toRadians(rndrot);
                            float angle2 = G3D::toRadians(rndrot2);

                            // Calculate sine and cosine of half angles
                            float cosHalfAngle1 = cosf(angle1 * 0.5f);
                            float sinHalfAngle1 = sinf(angle1 * 0.5f);
                            float cosHalfAngle2 = cosf(angle2 * 0.5f);
                            float sinHalfAngle2 = sinf(angle2 * 0.5f);

                            // Construct quaternion directly from rotation angles
                            G3D::Quat rndQuat(
                                cosHalfAngle1 * cosHalfAngle2,
                                sinHalfAngle1 * cosHalfAngle2,
                                cosHalfAngle1 * sinHalfAngle2,
                                sinHalfAngle1 * sinHalfAngle2
                            );

                            // Extract components of the quaternion for rotation
                            float rotation0 = rndQuat[0];
                            float rotation1 = rndQuat[1];
                            float rotation2 = rndQuat[2];
                            float rotation3 = rndQuat[3];

                            // Check if _big is true before summoning the game object
                            if (_big)
                            {
                                if (GameObject* firework = me->SummonGameObject(FireworksBIGOnlyPicker(), posX, posY, posZ, 0.0f, rotation0, rotation1, rotation2, rotation3, 300, false))
                                {
                            ...
                            {
                                if (GameObject* firework = me->SummonGameObject(FireworksPicker(), posX, posY, posZ, 0.0f, rotation0, rotation1, rotation2, rotation3, 300, false))
                                {
                                    firework->SetRespawnTime(0);
                                    firework->Delete();
                                }
